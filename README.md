...
- 承認済みの参加希望に紐づく1対1チャット
- リアルタイムなしの掲示板型メッセージ機能
- テキストのみ対応
- ブロック関係がある場合は送信不可
- チャットメッセージ通報に対応
...
- Web版・モバイル版からログイン済みユーザーが送信可能
- カテゴリ、件名、内容、プラットフォーム、送信元画面をPostgreSQLへ保存
- メール送信、Push通知、外部問い合わせサービスは不使用
- 管理者がWeb管理画面で一覧・詳細を確認可能
- 管理者はステータスと管理者メモを管理可能
- 一般ユーザーは自分の送信履歴だけ閲覧可能
- 管理者メモは一般ユーザーに非公開
- 通報機能とは別モデル・別画面で管理
...
- パスワードユーザーは現在のパスワード確認が必要
- 確認文言「アカウントを削除」と最終確認が必要
- GoogleログインユーザーはWebではGoogle再ログイン、モバイルではログアウト後の再ログインにより、10分以内の再認証状態でのみ削除可能
- Web版・モバイル版に対応
- プロフィール、卓球記録、用具、練習メニュー、募集、参加希望、チャット、通知、通報、ブロック、フィードバックなど関連するユーザーデータを削除
- プロフィール画像は現状DB保存のdata URLまたは外部URL参照であり、DB削除により参照を削除。専用外部ストレージが設定された場合は安全なキー抽出を前提に処理する方針
- 削除後はWebセッションのUser存在確認とモバイルBearer tokenのUser存在確認により、削除済みUserの認証状態を無効化
- 原則として復元不可
- バックアップやログに一定期間残る可能性あり
...
- 新しいチャットメッセージをWeb版・モバイル版のアプリ内通知へ表示
- 通知から該当チャットへ移動可能
- 通知単体・一括既読に対応
- チャット詳細を開くと関連通知も既読化
- Push通知・リアルタイム通信は未対応
- 通知へチャット全文や秘密情報を保存しない
