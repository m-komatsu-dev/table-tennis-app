...
卓球プレイヤー向けに、練習記録・試合記録・用具・プロフィール・統計を管理できるWebアプリです。記録データから課題を分析し、次回の練習メニューを提案するGemini AIコーチも利用できます。モバイル版Phase 1として、Expo Goで動く記録閲覧・作成アプリも含みます。
...
- npm workspaces
- Vitest
- Expo / React Native / Expo Router（モバイル版）
...
MOBILE_AUTH_SECRET="モバイルBearer Token署名用の十分に長いランダム文字列"
...
モバイルアプリは `apps/mobile` にあります。Expo / React Native / TypeScript / Expo Router で実装されており、DBへ直接接続せず、`apps/web` の `/api/mobile/...` を呼び出します。

モバイル認証はWeb版のCookieセッションとは別に、Phase 1用のBearer Token方式です。Web API側では `MOBILE_AUTH_SECRET` を使って署名し、各 mobile API で `Authorization: Bearer <accessToken>` を検証します。`MOBILE_AUTH_SECRET` が未設定、または短すぎる場合は安全にエラーになります。

ローカルでWeb APIとExpoを起動します。
...
本番APIへ接続する場合は次のように設定します。

```env
EXPO_PUBLIC_API_URL="https://table-tennis-app-rho.vercel.app"
```

Expo Goで表示されたQRコードを読み取り、Web版に登録済みのメールアドレスとパスワードでログインしてください。Phase 1ではログイン、ログアウト、ホーム、練習記録一覧・作成、試合記録一覧・作成、練習メニュー一覧、プロフィール表示を確認できます。
